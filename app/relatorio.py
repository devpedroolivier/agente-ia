...
def gerar_grafico_por_polo(dados, polo=None, polos=None, dias_intervalo=1, caminho_saida=None):
    try:
        ...
        # 📊 Caso 1: 1 CEO + 1 dia → gráfico por setor em blocos
        ...
        # 📊 Caso 2: Múltiplos CEOs → gráfico único por CEO
        if polos and len(polos) > 1:
            if dias_intervalo == 1:
                ...
                plt.title("Reclamações por CEO (último dia)", fontsize=14, fontweight='bold')
                plt.xlabel("CEO")
                plt.ylabel("Qtd de Reclamações")
